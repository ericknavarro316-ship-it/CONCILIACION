...
## 2026-04-26 - Enhance Streamlit action buttons UX
**Learning:** Long-running backend operations tied to `st.button`s lack visual feedback by default, and grouped buttons look misaligned without explicit width constraints.
**Action:** Wrap long-running execution logic in `st.spinner()` for feedback, provide contextual tooltips using the native `help` parameter, and improve layout alignment by applying `width='stretch'` on grouped action buttons.
