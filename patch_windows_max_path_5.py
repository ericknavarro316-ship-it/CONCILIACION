...
# Double backslash escaping in normal string parsing is tricky, using explicit characters
...
search_process = r"""                        os.makedirs(ruta_base, exist_ok=True)
...
replace_process = r"""                        os.makedirs(safe_path(ruta_base), exist_ok=True)
                        safe_name = re.sub(r'[^a-zA-Z0-9_\-\.]', '_', file_name)
                        ruta_destino = os.path.join(ruta_base, safe_name)
...
search_ventas_zip = r"""                                    # Guardar archivo
...
replace_ventas_zip = r"""                                    # Guardar archivo
...
search_ventas_pdf = r"""                            # 3. Guardar archivo
                            os.makedirs(ruta_base, exist_ok=True)
...
replace_ventas_pdf = r"""                            # 3. Guardar archivo
                            os.makedirs(safe_path(ruta_base), exist_ok=True)
                            safe_name = re.sub(r'[^a-zA-Z0-9_\-\.]', '_', pdf_file.name)
                            ruta_destino = os.path.join(ruta_base, safe_name)
